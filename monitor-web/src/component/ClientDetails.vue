...
const memoryTotal = computed(() => {
  const runtime = toPositiveNumber(details.runtime?.memory)
  if(runtime)
    return runtime
  return toPositiveNumber(details.base?.memory)
})
const diskTotal = computed(() => {
  const runtime = toPositiveNumber(details.runtime?.disk)
  if(runtime)
    return runtime
  return toPositiveNumber(details.base?.disk)
})
